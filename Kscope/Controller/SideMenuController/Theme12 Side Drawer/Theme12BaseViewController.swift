//
//  Theme12BaseViewController.swift
//

import UIKit

class Theme12BaseViewController: UIViewController, Theme12SlideMenuDelegate {
    
    var sideDrawerIndex = Int()
    ...
    func addSlideMenuButton(index: Int){
        sideDrawerIndex = index
        let btnShowMenu = UIButton(type: UIButton.ButtonType.system)
        btnShowMenu.setImage(self.defaultMenuImage(), for: UIControl.State())
        btnShowMenu.frame = CGRect(x: 16, y: 28, width: 30, height: 30)
        btnShowMenu.addTarget(self, action: #selector(Theme12BaseViewController.onSlideMenuButtonPressed(_:)), for: UIControl.Event.touchUpInside)
    ...
    @objc func onBack_Pressed(_ sender : UIButton){
